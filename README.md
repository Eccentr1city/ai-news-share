...
The NYT one is wired in (`src/ai_news_share/nyt.py`): put `NYT_API_KEY=...` in a
git-ignored `.env` (see `.env.example`) or the environment, run
`uv run ai-news-share nyt`, and a "NYT print front page" source appears in the
