...
def cmd_report(args) -> None:
    ...
    fk_ai = getattr(args, "fit_window_ai", None) or fk
    print(f"  Doubling time of the odds ({fk}-day window for Covid, {fk_ai}-day for AI), least squares on ln(odds):")
    lo_cv = analysis.pooled_log_odds(daily, "covid", fk)
    lo_ai = analysis.pooled_log_odds(daily, "ai", fk_ai)
    last = days[-1]
